...
import { type ActiveSessionContext, type WeekSummary } from '../lib/sessionsApi';
...
function accentFor(dayName: string): string {
  return ACCENTS[dayName] ?? ACCENTS[dayName.replace(/\s+\d+$/, '')] ?? FALLBACK_ACCENT;
}
/**
  ...
  return (
    <h1 className="text-display font-bold leading-tight tracking-[-0.02em] text-ink">
      {name == null ? (
        ...
        <>
          {hello},{hello.includes(' ') ? <span className="block">{name}.</span> : <> {name}.</>}
        </>
      )}
    </h1>
  );
...
export function Home({
  ...
  const [lastCompleted, setLastCompleted] = useState<string | null>(initial?.lastCompleted ?? null);
  ...
  const [active, setActive] = useState<ActiveSessionContext | null>(initial?.active ?? null);
  ...
  const [recentPositions, setRecentPositions] = useState<number[]>(initial?.recentPositions ?? []);
  ...
  const nextSlotName = getNextDayName(
    mainSlots,
    lastCompleted ? baseDayName(lastCompleted) : null,
    completedSlotNames,
  );
  ...
  return (
    ...
    >
      ...
      <span
        ...
        style={wiggleKey > 0 ? { animation: 'reps-coffee-wiggle 450ms ease-out' } : undefined}
      >
        <CoffeeIcon />
      </span>
 ...
 */
function StepsAction({ count, goal, onTap }: { count: number; goal: number; onTap: () => void }) {
  ...
  return (
    ...
    >
      ...
      <span className="relative whitespace-nowrap tabular-nums">
        {formatSteps(count)}
        <span className="text-muted">{reached ? ' \u2713' : ` / ${formatSteps(goal)}`}</span>
      </span>
    </button>
  );
}
